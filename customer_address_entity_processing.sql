...
--CREATING A STAGE LEVEL TABLE--
create or replace table stage_schema.customeraddress (
...
---CREATING A STAGE LEVEL STREAM
create or replace stream stage_schema.customeraddress_stm 
on table stage_schema.customeraddress
append_only = true;

--COPYING DATA INTO STREAM--
copy into stage_schema.customeraddress (addressid, customerid, flatno, houseno, floor, building, 
...
from (
    ...
    from @stage_schema.csv_stg/initial/customer_address t
)
file_format = (format_name = 'stage_schema.csv_file_format')
on_error = abort_statement;

--CREATING A CLEAN LEVEL TABLE
CREATE OR REPLACE TABLE CLEAN_SCHEMA.CUSTOMER_ADDRESS (
...
---SCHEMA CUSTOMER_ADDRESS_TABLE_CLEAN_SCHEMA_TABLE------------------
create or replace stream CLEAN_SCHEMA.CUSTOMER_ADDRESS_STM
on table CLEAN_SCHEMA.CUSTOMER_ADDRESS;



--MERGE INTO CLEAN LEVEL SCHEMA--
MERGE INTO clean_schema.customer_address AS clean
USING (
    SELECT 
        ...
        _stg_file_md5,
        _copy_data_ts
    FROM stage_schema.customeraddress_stm 
) AS stage
...
----CREATING CONSUMPTION/DIMENSION TABLE
CREATE OR REPLACE TABLE CONSUMPTION_SCHEMA.CUSTOMER_ADDRESS_DIM (
...
--MERGE INTO CONSUMPTION SCHEMA
MERGE INTO 
    CONSUMPTION_SCHEMA.CUSTOMER_ADDRESS_DIM AS target
USING 
    CLEAN_SCHEMA.CUSTOMER_ADDRESS_STM AS source
ON 
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @stage_schema.csv_stg/delta/customer_address/ t
)
file_format = (format_name = 'stage_schEMA.csv_file_format')
on_error = abort_statement;

select * from CONSUMPTION_SCHEMA.CUSTOMER_ADDRESS_DIM;
