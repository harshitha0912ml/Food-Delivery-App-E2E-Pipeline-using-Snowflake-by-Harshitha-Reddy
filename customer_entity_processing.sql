...
create or replace table stage_schema.customer (
...
create or replace stream stage_schema.customer_stm 
on table stage_schema.customer
append_only = true;

--COPY INTO COMMAND--

copy into  stage_schema.customer (customerid, name, mobile, email, loginbyusing, gender, dob, anniversary, 
...
from (
    ...
    from @stage_schema.csv_stg/delta/customers/day-02-insert-update.csv t
)
file_format = (format_name = 'stage_schema.csv_file_format')
on_error = abort_statement;


CREATE OR REPLACE TABLE CLEAN_SCHEMA.CUSTOMER (
...
create or replace stream CLEAN_SCHEMA.customer_stm 
on table CLEAN_SCHema.customer;


select * from clean_schema.customer;


---MERGE INTO CLEAN SCHEMA=----
MERGE INTO CLEAN_SCHEMA.CUSTOMER AS target
USING (
    SELECT 
        ...
        _STG_FILE_MD5,
        _COPY_DATA_TS
    FROM STAGE_SCHEMA.CUSTOMER_STM
) AS source
...
select count(*) from CLEAN_SCHEMA.CUSTOMER;
select count(*) from CLEAN_SCHEMA.CUSTOMER_STM;


-- create dim table 
CREATE OR REPLACE TABLE CONSUMPTION_SCHEMA.CUSTOMER_DIM (
...
MERGE INTO 
    CONSUMPTION_SCHEMA.CUSTOMER_DIM AS target
USING 
    CLEAN_SCHEMA.CUSTOMER_STM AS source
ON 
...
---
list @stage_schema.csv_stg/delta/customers/;

select * from consumption_schema.customer_dim;